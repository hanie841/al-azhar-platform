    ...
    <div className="min-h-screen bg-sand-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          {stats.map((stat, i) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-white rounded-2xl p-6 text-center shadow-sm border border-sand-100"
              >
                <Icon className="w-8 h-8 text-accent-500 mx-auto mb-3" />
                <div className="text-3xl font-bold text-primary-900 mb-1">{stat.value}</div>
                <div className="text-sm text-sand-600">{stat.label}</div>
              </motion.div>
            );
          })}
        </div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
          viewport={{ once: true }}
          className="bg-white rounded-2xl p-8 lg:p-12 shadow-sm border border-sand-100 mb-20"
        >
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-primary-900 mb-6 text-center">
            {t('reachTitle')}
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {regions.map((region, i) => (
              ...
              >
                <h3 className="font-serif font-bold text-primary-900 mb-2">{region.name}</h3>
        ...
        <motion.div
          ...
          className="mb-20"
        >
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-primary-900 mb-8 text-center">
            {t('partnersTitle')}
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-white rounded-xl p-4 text-center border border-sand-100 hover:border-primary-200 transition-colors"
              >
