      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              {person.era && (
                <span className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-navy-800 rounded-xl border border-sand-200 dark:border-navy-700 text-sm font-medium text-sand-700 shadow-sm dark:shadow-navy-900/50">
                  <Clock className="w-4 h-4 text-accent-600" />
                  {person.era}
                </span>
              )}
              {person.is_current_leadership && (
                <span className="inline-flex items-center gap-2 px-4 py-2 bg-accent-50 rounded-xl border border-accent-200 text-sm font-medium text-accent-700 shadow-sm dark:shadow-navy-900/50">
                  <Crown className="w-4 h-4 text-accent-600" />
                  {isAr ? 'القيادة الحالية' : 'Current Leadership'}
                </span>
              )}
              {person.is_historical && (
                <span className="inline-flex items-center gap-2 px-4 py-2 bg-sand-100 dark:bg-navy-800 rounded-xl border border-sand-200 dark:border-navy-700 text-sm font-medium text-sand-700 shadow-sm dark:shadow-navy-900/50">
                  <BookOpen className="w-4 h-4 text-sand-500 dark:text-sand-400" />
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-navy-800 rounded-2xl p-8 shadow-sm dark:shadow-navy-900/50 flex flex-col items-center"
            >
              <div
              ...
              <h3 className="font-serif text-lg font-bold text-primary-900 dark:text-primary-200 text-center">
                {person.name}
              </h3>
              ...
              <div className="space-y-4">
                {person.era && (
                  <div className="flex items-center gap-3 text-sm text-sand-600 dark:text-sand-400">
                    ...
                    <div>
                      <p className="text-xs text-sand-400 dark:text-sand-500">{isAr ? 'الحقبة' : 'Era'}</p>
                ...
                <div className="flex items-center gap-3 text-sm text-sand-600 dark:text-sand-400">
                  ...
                  <div>
                    <p className="text-xs text-sand-400 dark:text-sand-500">{isAr ? 'الحالة' : 'Status'}</p>
